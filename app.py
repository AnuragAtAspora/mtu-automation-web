UK_All_Users_0205_1430#!/usr/bin/env python3
"""
...
class MTUWebAutomation:
    ...
    def create_segment(self, segment_name, description, filters):
        """Create a segment using MoEngage Custom Segment API with simple naming and duplicate handling"""
        try:
            ...
            # Use simple segment name as provided
            payload = {
                "name": segment_name,
...
def generate_metrics():
    """Create segments only - campaign data fetched later"""
    try:
        ...
        # Only create 16 segments - no campaign data fetching yet
...
def calculate_final_metrics():
    ...
    try:
        ...
        start_date = request.form.get('start_date')
        end_date = request.form.get('end_date')
        segment_ids = request.form.get('segment_ids', '').split(',') if request.form.get('segment_ids') else []
        ...
        # NOW fetch campaign data (after user has entered segment counts)
...
def get_campaign_performance_data(start_date, end_date):
    ...
    try:
        ...
        # Fetch all campaigns with metadata (limit to 50 pages for web performance)
        campaigns = fetcher.fetch_all_campaigns(start_date, end_date, max_pages=50, fetch_meta=True)
